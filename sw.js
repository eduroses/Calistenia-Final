const CACHE='calistenia-militar-final-v12';
...
const PATCH = `<style id="cal-v12-style">.pdf-caption{display:none!important}.exercise-visual img{image-rendering:auto!important}</style><script id="cal-v12-patch">(()=>{
'use strict';
...
function nextExercise(e){
 ...
 if(!label.includes('siguiente ejercicio'))return;
 const items=Array.from(document.querySelectorAll('.work-item'));
 if(!items.length)return;
 const active=document.querySelector('.work-item.active');
 let idx=items.indexOf(active); if(idx<0)idx=0;
 const next=items[idx+1];
 if(next){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();next.click();}
}
document.addEventListener('click',nextExercise,true);
...
async function patch(response){
 ...
 try{
  ...
  if(!text.includes('cal-v12-patch'))text=text.replace('</body>',PATCH+'</body>');
