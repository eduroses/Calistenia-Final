const CACHE='calistenia-militar-final-v9';
...
const PATCH = `<script id="calistenia-v9-patch">
(()=>{
  'use strict';
  let audioCtx=null;
  ...
  function tone(freq,duration=.055,gain=.035,type='sine'){
    ...
    try{
      ...
      o.start(); o.stop(audioCtx.currentTime+duration+.015);
    }catch(e){}
  }
  function finishTone(){
    tone(880,.12,.055,'sine');
    setTimeout(()=>tone(1175,.18,.055,'sine'),140);
  }

  // Mobile browsers require a user gesture before Web Audio can play.
  document.addEventListener('pointerdown',unlockAudio,{passive:true});
  document.addEventListener('touchstart',unlockAudio,{passive:true});
  document.addEventListener('click',()=>{unlockAudio();setTimeout(()=>{if(soundOn())tone(620,.045,.025)},30)},{passive:true});
  ...
  // The original app updates the visible clock once per second. We monitor the
  // displayed value instead of replacing its timer logic, so work/rest timers
  // and future timer changes all receive the same audio treatment.
  function checkTimer(){
  ...
  function cleanCaptions(){
    document.querySelectorAll('.pdf-caption').forEach(el=>el.remove());
  }
...
async function patchHtml(response){
  ...
  try{
    ...
    if(text.includes('calistenia-v9-patch')) return new Response(text,{status:response.status,statusText:response.statusText,headers:response.headers});
    const patched=text
      .replaceAll('./assets/exercises/','./')
      .replace(/<div class="pdf-caption">[\\s\\S]*?<\\/div>/g,'')
...
self.addEventListener('fetch',event=>{
  ...
  if(url.pathname.includes('/assets/exercises/')){
    ...
    fixed.pathname=url.pathname.substring(0,url.pathname.indexOf('/assets/exercises/'))+'/'+filename;
    event.respondWith(
      fetch(fixed).then(r=>r).catch(()=>caches.match(fixed).then(r=>r||caches.match(event.request)))
    );
    return;
  }
  event.respondWith(
    fetch(event.request).then(r=>patchHtml(r)).catch(()=>caches.match(event.request).then(r=>r||caches.match('./index.html')))
  );
});
